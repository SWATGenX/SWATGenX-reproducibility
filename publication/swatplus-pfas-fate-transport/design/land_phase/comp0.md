Everything works end to end: database parsed (2 compounds, terminator-stopped), per-HRU allocation off `soil(ihru)%nly`, num_pconta 0→1 coercion, d50 per-layer, the 1.e-9 microg/ha→kg/ha conversion (1000 → 9.999...e-7), kf/nf/enr stored, and the `pfasid<=0` sentinel correctly flags HRU2 as PFAS-free.

The deliverable is at `/data/SWATGenXApp/codes/_temp/pfas-swatplus-port/engine/src/pfas_read.f90`. Full source follows.
...
!!    ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
      ...
      do
        ...
        do ihru = 1, nhru
          ...
          allocate (xx(nly), source = 0.)

          !! per-HRU name/header line: "hru <id>  nly <nly>" (skipped)
          ...
          !! median grain diameter d50 (mm), one value per layer
          read (171,*,iostat=eof) (xx(ly), ly = 1, nly)
          if (eof < 0) exit
          do ly = 1, nly
          ...
          do k = 1, npfas
            ...
            read (171,*,iostat=eof) pfasid, (xx(ly), ly = 1, nly)
            if (eof < 0) exit
            if (pfasid <= 0) then
     ...
     &                                              1.e-9 * xx(ly)
            ...
            read (171,*,iostat=eof) id, (xx(ly), ly = 1, nly)
            if (eof < 0) exit
            do ly = 1, nly
...
- File 1 `pfas.dat`: title + header + one record per compound (`id name mw sol kl lm percop`), id<=0 terminates — mirrors readpfas.f.
- File 2 `pfas_hru.ini`: consolidated single-file init (modern pest_hru.ini/cs_hru.ini style) rather than the SWAT2012 per-HRU `.chm`; one record set per HRU in HRU order: name line, num_pconta, per-layer d50, then per-PFAS quadruples (sol_pfas / kf / nf / enr). `pfasid<=0` is the per-HRU "no PFAS" sentinel.
- Byte-faithful 1.e-9 microg/ha→kg/ha conversion on `sol_pfas`, and 0→1 coercion of `num_pconta`, both from readchm.f.
- Allocation sized off `sp_ob%hru` and `soil(ihru)%nly`; per-layer constituent vectors sized by `npfas`; sets `pfas_flag`, `pfas_num`, `npfas`.
- Compile-verified `gfortran -ffree-form -c` against `pfas_module.f90` (and engine module stubs), plus an end-to-end functional run confirming parsing, allocation, unit conversion, and the no-PFAS sentinel.